...
from .config import EdgeConfig, WindowConstructionConfig
...
def _build_support_band(values: np.ndarray, peak_info: _PeakInfo, cfg: WindowConstructionConfig) -> tuple[int, int, float]:
    n = values.size
...
def _consolidate_bands(bands_df: pd.DataFrame, profile: pd.Series, cfg: WindowConstructionConfig) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if bands_df.empty:
        return pd.DataFrame(columns=cols), pd.DataFrame(columns=['merged_window_id', 'source_point_days', 'source_band_starts', 'source_band_ends', 'n_merged_points'])
    ...
    return pd.DataFrame(windows, columns=cols), pd.DataFrame(merge_rows)


def build_primary_windows_from_ruptures(points: pd.Series, profile: pd.Series, edge_cfg: EdgeConfig, cfg: WindowConstructionConfig) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    if profile.empty:
        empty_cols = ['window_id', 'backend', 'window_type', 'start_day', 'end_day', 'center_day', 'main_peak_day', 'native_support', 'support_points', 'band_strength', 'n_support_points']
        return (
    ...
    if not point_df.empty and not primary_windows.empty:
        for _, row in point_df.iterrows():
            ...
            assigned = None if matched.empty else str(matched.iloc[0]['window_id'])
            r = row.to_dict()
...
def build_edge_windows_from_ruptures(profile: pd.Series, primary_windows: pd.DataFrame, edge_cfg: EdgeConfig, cfg: WindowConstructionConfig) -> pd.DataFrame:
    cols = ['window_id', 'backend', 'window_type', 'start_day', 'end_day', 'center_day', 'main_peak_day', 'native_support', 'support_points', 'band_strength', 'n_support_points']
    if profile.empty:
    ...
    for i, p in enumerate(peaks):
        ...
        if info.prominence < prom_threshold:
            continue
        left, right, _ = _build_support_band(values, info, cfg)
        edge_idx += 1
        rows.append({
...
def build_point_table(primary_points: pd.Series, point_to_window_df: pd.DataFrame) -> pd.DataFrame:
    rows = []
    if not point_to_window_df.empty:
        ordered = point_to_window_df.copy().sort_values(['peak_day']).reset_index(drop=True)
        for i, row in ordered.iterrows():
            rows.append({
                'point_id': _format_id('RP', i + 1),
                'point_day': int(row['peak_day']),
                'peak_value': float(row['peak_value']),
                'peak_prominence': float(row['peak_prominence']),
                'peak_width': float(row['peak_width']),
                'assigned_window_id_legacy': row.get('assigned_window_id'),
            })
    elif not primary_points.empty:
        ordered = primary_points.astype(int).sort_values().reset_index(drop=True)
        for i, point_day in enumerate(ordered.tolist()):
            rows.append({
                'point_id': _format_id('RP', i + 1),
...
def build_band_table(point_to_window_df: pd.DataFrame) -> pd.DataFrame:
    cols = ['band_id', 'band_start_day', 'band_end_day', 'band_threshold', 'source_window_id_legacy']
    if point_to_window_df.empty:
        return pd.DataFrame(columns=cols)
    bands = point_to_window_df[['band_start_day', 'band_end_day', 'band_threshold', 'assigned_window_id']].drop_duplicates().reset_index(drop=True)
    rows = []
    for i, row in bands.iterrows():
        rows.append({
            'band_id': _format_id('RB', i + 1),
            'band_start_day': int(row['band_start_day']),
            'band_end_day': int(row['band_end_day']),
            'band_threshold': float(row['band_threshold']),
            'source_window_id_legacy': row.get('assigned_window_id'),
        })
    return pd.DataFrame(rows, columns=cols)


def build_main_window_table(primary_windows_df: pd.DataFrame, point_df: pd.DataFrame, band_df: pd.DataFrame) -> pd.DataFrame:
    cols = [
    ...
    if primary_windows_df.empty:
        return pd.DataFrame(columns=cols)
    ordered = primary_windows_df.sort_values(['center_day', 'start_day', 'end_day']).reset_index(drop=True)
    rows = []
    for i, row in ordered.iterrows():
        legacy_id = str(row['window_id'])
        start_day = int(row['start_day'])
        end_day = int(row['end_day'])
        points = point_df[point_df['assigned_window_id_legacy'] == legacy_id]['point_id'].astype(str).tolist() if not point_df.empty else []
        if not points and not point_df.empty:
            points = point_df[point_df['point_day'].between(start_day, end_day, inclusive='both')]['point_id'].astype(str).tolist()
        bands = band_df[band_df['source_window_id_legacy'] == legacy_id]['band_id'].astype(str).tolist() if not band_df.empty else []
        if not bands and not band_df.empty:
            overlap = (band_df['band_start_day'] <= end_day) & (band_df['band_end_day'] >= start_day)
            bands = band_df.loc[overlap, 'band_id'].astype(str).tolist()
        rows.append({
            ...
            'origin_detector': str(row.get('backend', 'ruptures_window')),
            'window_build_version': 'window_native_v3_a',
            'source_primary_point_ids': ','.join(points),
            'source_support_band_ids': ','.join(bands),
            'is_main_window': True,
            'window_status': 'main',
            'status_reason': 'from_current_v3_ruptures_primary_windows',
            'catalog_match_status': 'advisory_only',
            'notes': 'V3 primary main-window table is the only formal downstream window-object source',
        })
    return pd.DataFrame(rows, columns=cols)


def build_window_catalog(primary_windows_df: pd.DataFrame, edge_windows_df: pd.DataFrame) -> pd.DataFrame:
    frames = []
    if not primary_windows_df.empty:
        out = primary_windows_df.copy()
        out['support_class'] = 'primary_ruptures'
        frames.append(out)
    if not edge_windows_df.empty:
        out = edge_windows_df.copy()
