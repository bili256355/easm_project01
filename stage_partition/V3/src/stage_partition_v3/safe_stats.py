...
def safe_nanmean(arr: np.ndarray, axis, return_valid_count: bool = False):
    """Mean ignoring NaN without RuntimeWarning on all-NaN slices."""
...
def safe_daily_energy(arr: np.ndarray) -> dict[str, float]:
    ...
    daily_energy_mean = float(np.mean(day_mean[finite_day])) if np.any(finite_day) else np.nan
    finite_sq = np.isfinite(sq)
    rms = float(np.sqrt(np.mean(sq[finite_sq]))) if np.any(finite_sq) else np.nan
