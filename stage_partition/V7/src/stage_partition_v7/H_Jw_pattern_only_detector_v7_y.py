...
OUTPUT_TAG = "H_Jw_pattern_only_detector_v7_y"
VERSION = "v7_y"
WINDOW_ID = "W002"
ANCHOR_DAY = 45
SYSTEM_W45 = (40, 48)
...
def _load_npz(path: Path) -> dict[str, Any]:
    ...
    with np.load(path, allow_pickle=False) as data:
        ...
        if missing:
            raise KeyError(f"smoothed_fields.npz is missing required keys for V7-y: {missing}")
        return {k: data[k] for k in data.files}


def _resolve_paths(v7_root: Optional[Path], cfg: V7YConfig) -> V7YPaths:
    if v7_root is None:
    ...
    for p in (output_dir, log_dir, figure_dir):
        _ensure_dir(p)
    return V7YPaths(v7_root=v7_root, project_root=project_root, output_dir=output_dir, log_dir=log_dir, figure_dir=figure_dir)
...
def _build_object_profile(smoothed: dict[str, Any], spec: ObjectSpec, cfg: V7YConfig) -> PointLayerProfile:
...
def _zscore_along_day(x: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    # Retained for compatibility only; V7-y does not use full-season z-score as detector input.
...
def _lat_weights(lat_grid: np.ndarray) -> np.ndarray:
    ...
    if not np.isfinite(w).any():
        return np.ones_like(np.asarray(lat_grid, dtype=float))
    w = w / np.nanmean(w)
...
def _weighted_mean_1d(x: np.ndarray, w: np.ndarray) -> float:
    ...
    if m.sum() == 0:
        return float("nan")
    return float(np.nansum(x[m] * w[m]) / np.nansum(w[m]))


def _shape_normalize_profiles(cube: np.ndarray, lat_grid: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Yearwise/daywise shape normalization of profile cube.

    cube: years x days x lat_feature. For each year/day, remove weighted lat mean
    and divide by weighted lat norm. Multiplying by sqrt weights makes ordinary L2
    in ruptures closer to a weighted pattern distance.
    """
    ...
    for i in range(cube.shape[0]):
        for j in range(cube.shape[1]):
            row = cube[i, j, :].astype(float)
            good = np.isfinite(row) & np.isfinite(w)
            if good.sum() < 2:
                continue
            mu = _weighted_mean_1d(row, w)
            centered = row - mu
            norm = float(np.sqrt(np.nansum(w[good] * centered[good] ** 2) / np.nansum(w[good])))
            norms[i, j] = norm
...
def _build_object_state(profile: PointLayerProfile, sampled_year_indices: Optional[np.ndarray] = None) -> dict[str, Any]:
    ...
    pattern_cube, norm_cube, pattern_valid = _shape_normalize_profiles(cube, profile.lat_grid)
    seasonal, valid_count = _safe_nanmean(pattern_cube, axis=0, return_valid_count=True)
    state_matrix = np.asarray(seasonal, dtype=float)
    ...
    for j, latv in enumerate(profile.lat_grid):
        ...
        feature_rows.append({
            ...
            "input_type": "shape_normalized_profile",
            "n_valid_days_shape_pattern": int(np.sum(np.isfinite(col))),
            "mean_shape_pattern_value": float(np.nanmean(col)) if np.isfinite(col).any() else np.nan,
            "std_shape_pattern_value": float(np.nanstd(col)) if np.isfinite(col).any() else np.nan,
        })
    return {
        "raw_matrix": state_matrix,
        ...
        "shape_norm_summary": {
            "mean_norm": float(np.nanmean(norm_cube)) if np.isfinite(norm_cube).any() else np.nan,
            "min_norm": float(np.nanmin(norm_cube)) if np.isfinite(norm_cube).any() else np.nan,
            "max_norm": float(np.nanmax(norm_cube)) if np.isfinite(norm_cube).any() else np.nan,
            "valid_pattern_fraction": float(np.mean(pattern_valid)) if pattern_valid.size else np.nan,
        },
    }
...
def _import_ruptures():
    try:
        ...
        raise ImportError("V7-y requires the ruptures package in the user's environment.") from exc
...
def _extract_ranked_local_peaks(profile: pd.Series, min_distance_days: int, prominence_min: float = 0.0) -> pd.DataFrame:
    ...
    if _SCIPY_IMPORT_ERROR is not None:  # pragma: no cover
        raise ImportError("V7-y requires scipy.signal for local peak extraction.") from _SCIPY_IMPORT_ERROR
...
def _run_object_detector(state: dict[str, Any], cfg: V7YConfig) -> dict[str, Any]:
...
def _build_object_outputs(obj: str, profile: PointLayerProfile, years: np.ndarray, cfg: V7YConfig) -> dict[str, pd.DataFrame | dict[str, Any]]:
    ...
    debug_n = os.environ.get("V7Y_DEBUG_N_BOOTSTRAP")
    ...
    for rep in _progress_iter(range(n_boot), progress_enabled, f"V7-y {obj} bootstrap"):
...
def _classify_window_status(x: float) -> str:
    ...
    if float(x) >= 0.95:
        return "accepted_pattern_window_95"
    if float(x) >= 0.80:
        return "candidate_pattern_window_80"
    if float(x) >= 0.50:
        return "weak_pattern_window_50"
    return "unstable_pattern_window"
...
def _build_summary_json(outputs: dict[str, pd.DataFrame], detector_meta: list[dict[str, Any]], cfg: V7YConfig) -> dict[str, Any]:
    ...
    for obj in cfg.objects:
        ...
        by_obj[obj] = {
            ...
            "n_accepted_95_windows": int((subw.get("window_status", pd.Series(dtype=str)) == "accepted_pattern_window_95").sum()) if not subw.empty else 0,
...
def _build_markdown_summary(summary: dict[str, Any], cfg: V7YConfig) -> str:
    lines = []
    lines.append("# V7-y H/Jw pattern-only shape-normalized V6_1-style detector")
    lines.append("")
    lines.append("## Purpose")
    lines.append("Test whether H/Jw shape-normalized profile patterns contain object-level pattern windows over day0-70 while reusing the original W45 detector skeleton.")
    lines.append("")
    lines.append("## What was changed relative to system W45")
    lines.append("- object_scope: H-only and Jw-only runs")
    lines.append("- detector_day_range: day0-70")
    lines.append("- detector input: raw profile state -> shape-normalized profile pattern")
    lines.append("")
    lines.append("## What was not changed")
    lines.append("- profile representation: 2-degree latitudinal profiles")
    lines.append("- yearwise/daywise shape normalization before day0-70 detection")
    ...
    lines.append("This run identifies shape-pattern detector peaks/windows. It does not infer causality, physical mechanism, or H/Jw pathway order.")
    return "\n".join(lines)


def _try_write_figures(outputs: dict[str, pd.DataFrame], paths: V7YPaths) -> None:
    if os.environ.get("V7Y_SKIP_FIGURES") == "1":
        return
    try:
    ...
    ax.set_title("H/Jw shape-pattern V6_1-style detector scores, day0-70")
    ax.legend(loc="best")
    fig.tight_layout()
    ...
    if windows is not None and not windows.empty:
        ...
        ax.set_xlabel("day index")
        ax.set_title("H/Jw shape-pattern candidate windows vs system W45")
        fig.tight_layout()
        fig.savefig(paths.figure_dir / "H_Jw_object_windows_vs_W45_v7_w.png", dpi=180)
        plt.close(fig)


def run_H_Jw_pattern_only_detector_v7_y(v7_root: Optional[Path] = None) -> dict[str, Any]:
    cfg = V7YConfig()
    debug_n = os.environ.get("V7Y_DEBUG_N_BOOTSTRAP")
    if debug_n:
        cfg = V7YConfig(bootstrap=BootstrapConfig(n_bootstrap=int(debug_n), progress=False))
    started_at = _now_iso()
    ...
    run_meta = {
        ...
        "version": VERSION,
        "output_tag": cfg.output_tag,
        "primary_goal": "shape-normalized pattern-only V6_1-style detector for H and Jw over day0-70",
        "system_W45_reference": {"window_id": WINDOW_ID, "anchor_day": ANCHOR_DAY, "accepted_window": list(SYSTEM_W45)},
        "method_reuse": "V6_1/W45 ruptures.Window + local peaks + candidate bands + bootstrap matching",
        "input_transformation": "yearwise daywise raw profile -> weighted lat-demeaned and weighted norm-normalized shape profile",
        ...
        "notes": [
            ...
            "This run intentionally reuses the original W45 detector skeleton while changing detector input to shape-normalized profile pattern.",
            "Shape-pattern windows are compared to system W45 day40-48 only after detection.",
        ],
    }
...
__all__ = ["run_H_Jw_pattern_only_detector_v7_y"]
