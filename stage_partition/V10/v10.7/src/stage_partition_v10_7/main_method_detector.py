...
from .utils import day_index_to_month_day
...
def _import_ruptures():
    try:
        ...
        raise ImportError(
            "ruptures is required for V10.7_a H-only main-method detector. "
            "Install/use the same environment that ran V10.2/V10.5."
        ) from e
...
def extract_ranked_local_peaks(profile: pd.Series, min_distance_days: int, prominence_min: float = 0.0) -> pd.DataFrame:
        ...
        rows.append(
            {
                ...
                "month_day": day_index_to_month_day(int(s.index[int(p)])),
    ...
    return df[["peak_id", "peak_day", "peak_score", "peak_prominence", "peak_rank", "source_type"]]
...
def run_ruptures_window(state_matrix: np.ndarray, cfg: DetectorConfig, width: int, day_index: np.ndarray | None = None) -> dict[str, Any]:
    rpt = _import_ruptures()
    signal = np.asarray(state_matrix, dtype=float)
    algo = rpt.Window(width=int(width), model=cfg.model, min_size=cfg.min_size, jump=cfg.jump).fit(signal)
...
def run_point_detector_for_width(state_matrix: np.ndarray, valid_day_index: np.ndarray, cfg: DetectorConfig, width: int) -> dict[str, Any]:
    out = run_ruptures_window(state_matrix, cfg, width=width, day_index=valid_day_index)
...
def profile_to_dataframe(profile: pd.Series, width: int) -> pd.DataFrame:
    if profile is None or profile.empty:
        return pd.DataFrame(columns=["detector_width", "day", "score"])
    s = profile.sort_index().astype(float)
    return pd.DataFrame({"detector_width": int(width), "day": s.index.astype(int), "score": s.to_numpy(dtype=float)})
