...
def _stage_root_from_v91b(v91b_root: Path) -> Path:
    return Path(v91b_root).resolve().parent
...
def _make_v7_cfg_for_v91b(v7multi, cfg91: V91BConfig, stage_root: Path):
...
def _window_scopes_from_v7(v7multi, stage_root: Path, cfg) -> List[object]:
    ...
    tmp = _ensure_dir(stage_root / "V9_1_b" / "outputs" / OUTPUT_TAG / "cross_window" / "_window_registry_tmp")
...
def run_transition_type_mixture_audit_v9_1_b(v91b_root: Path | str) -> None:
    v91b_root = Path(v91b_root)
    stage_root = _stage_root_from_v91b(v91b_root)
    cfg91 = V91BConfig.from_env()
    v7multi = _import_v7_module(stage_root)
    cfg_v7 = _make_v7_cfg_for_v91b(v7multi, cfg91, stage_root)

    out_root = _ensure_dir(v91b_root / "outputs" / OUTPUT_TAG)
    out_per = _ensure_dir(out_root / "per_window")
    out_cross = _ensure_dir(out_root / "cross_window")
    log_dir = _ensure_dir(v91b_root / "logs" / OUTPUT_TAG)
    t0 = time.time()
