...
def corr_matrix_batch(x: np.ndarray, y: np.ndarray, min_count: int = 3) -> tuple[np.ndarray, np.ndarray]:
    """
    Batched pairwise correlation matrices.

    Parameters
    ----------
    x, y : arrays with shape (B, N, V)
        Source and target arrays. NaNs are handled pairwise.
    Returns
    -------
    corr, count : arrays with shape (B, V, V)
        corr[:, i, j] = corr(x[:, :, i], y[:, :, j]).
    """
...
def estimate_ar1_params_diagnostic(
    a: np.ndarray,
    clip_limit: float = 0.95,
) -> dict:
    """
    Estimate AR(1) and return raw/clipped persistence diagnostics.

    Model:
        x_t - mu = phi * (x_{t-1} - mu) + eps

    Returns a dict including:
        mu, raw_phi_before_clip, phi_after_clip, sigma,
        phi_clipped_flag, phi_clip_amount, phi_clip_direction.
    """
    ...
    if den <= 0:
        ...
        raw_phi = float(np.sum(z0 * z1) / den)
...
def estimate_ar1_params(a: np.ndarray) -> tuple[float, float, float]:
    """
    Backward-compatible wrapper returning (mu, clipped_phi, sigma).
    Use estimate_ar1_params_diagnostic for raw phi audit.
    """
    diag = estimate_ar1_params_diagnostic(a)
    return float(diag["mu"]), float(diag["phi_after_clip"]), float(diag["sigma"])
