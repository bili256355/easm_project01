# -*- coding: utf-8 -*-
"""I/O helpers for T3 V→P field-explanation hard-evidence audit."""
...
from .t3_v_to_p_field_explanation_settings import FieldExplanationAuditSettings


def ensure_output_dirs(settings: FieldExplanationAuditSettings) -> Dict[str, Path]:
    dirs = {
        "root": settings.output_dir,
        "tables": settings.output_tables_dir,
        "figures": settings.output_figures_dir,
        "summary": settings.output_summary_dir,
    }
...
def load_index_values(settings: FieldExplanationAuditSettings) -> Tuple[pd.DataFrame, Path]:
    ...
    df = pd.read_csv(path)
    return normalize_year_day_columns(df), path


def load_smoothed_fields(settings: FieldExplanationAuditSettings) -> Tuple[Dict[str, np.ndarray], Path]:
...
def resolve_day_mapping(field: np.ndarray, data: Dict[str, np.ndarray]) -> Tuple[str, Dict[int, int]]:
    """Resolve day convention for a year x day x lat x lon field.

    Returns a mode string and dict mapping day-of-season (1=Apr 1) to field index.
    """
